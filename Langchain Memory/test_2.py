...
def get_messages_history_from_db(session_id):
    ...
    # Return the chat history (list of messages)
    return chat_history.messages  # This should return a list of messages
...
if user_id:
    ...
    if chat_history:
        for message in chat_history:
            # Debugging: print the message type and its attributes
            st.write(f"Message type: {type(message)}")
            st.write(f"Message content: {message}")

            # Check the type of message and handle accordingly
            if hasattr(message, 'text'):
                # If it has a 'text' attribute, it's likely a message with content
                st.chat_message("user" if isinstance(message, HumanMessage) else "assistant").write(message.text)
            else:
                # If the 'text' attribute is missing, we'll print the message's attributes
                st.write("Unknown message structure:", dir(message))

    # Show the greeting message after displaying all previous chat messages
